locals {
  resource_group_name = "tfs-rg-${terraform.workspace}-${local.region}-landingzone"
...
resource "azurerm_subnet" "inbound" {
  name                 = lower("tfs-snet-${terraform.workspace}-${var.subnet_in}-${local.region}")
...
resource "azurerm_subnet" "outbound" {
  name                 = lower("tfs-snet-${terraform.workspace}-${var.subnet_out}-${local.region}")
  virtual_network_name = local.vnet_name
  resource_group_name  = local.resource_group_name
...
resource "azurerm_network_security_group" "nsg_in" {
  name                = "tfs-nsg-${terraform.workspace}-${var.subnet_in}-${local.region}"
  resource_group_name = local.resource_group_name
...
resource "azurerm_route_table" "route_table_in" {
  name                = "tfs-rt-${terraform.workspace}-${var.subnet_in}-${local.region}"
  resource_group_name = local.resource_group_name
  location            = local.location
  depends_on = [
    azurerm_subnet.inbound
...
resource "azurerm_network_security_group" "nsg" {
  name                = "tfs-nsg-${terraform.workspace}-${var.subnet_out}-${local.region}"
  resource_group_name = local.resource_group_name
  location            = local.location
  depends_on = [
...
resource "azurerm_route_table" "route_table" {
  name                = "tfs-rt-${terraform.workspace}-${var.subnet_out}-${local.region}"
  resource_group_name = local.resource_group_name
  location            = local.location
  depends_on = [
    azurerm_subnet.outbound
...
resource "azurerm_private_dns_resolver" "private_dns_resolver" {
  name                = "tfs-${var.dns_resolver_name}-${terraform.workspace}-${local.region}"
  resource_group_name = local.resource_group_name
  location            = var.location
...
resource "azurerm_private_dns_resolver_dns_forwarding_ruleset" "forwarding_ruleset_outbound" {
  name                                       = "tfs-dns-fw-${var.forwarding_ruleset_outbound_name}-${terraform.workspace}-${local.region}"
  resource_group_name                        = local.resource_group_name
  location                                   = var.location
