...
resource "azurerm_private_dns_a_record" "a_record" {
  ...
  name                = "${terraform.workspace}-${var.dns_a_record_name}0${count.index + 1}.${var.product}.${terraform.workspace}.${local.loc_prefix}"
...
resource "azurerm_private_dns_cname_record" "cname_record" {
  provider            = azurerm.hub
  count               = length(var.count_of_vm)
  name                = "${terraform.workspace}.${var.dns_c_record_name}0${count.index + 1}.${var.product}.${terraform.workspace}.${local.loc_prefix}"
  zone_name           = data.azurerm_private_dns_zone.dns.name
  resource_group_name = data.azurerm_private_dns_zone.dns.resource_group_name
