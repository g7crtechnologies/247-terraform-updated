...
resource "azurerm_firewall_policy_rule_collection_group" "fw" {
  ...
  dynamic "network_rule_collection" {
    ...
    content {
      ...
      dynamic "rule" {
        for_each = [for r in var.network_rules : r if r.collection_name == network_rule_collection.value.name]
        content {
  ...
  dynamic "application_rule_collection" {
    ...
    content {
      name     = lower("tfs-fw-app-rule-${application_rule_collection.value.name}-${local.loc_prefix}")
      priority = application_rule_collection.value.priority
      action   = application_rule_collection.value.action
      dynamic "rule" {
        for_each = [for r in var.app_rules : r if r.collection_name == application_rule_collection.value.name]
        content {
          name                  = rule.value.name
  ...
  dynamic "nat_rule_collection" {
    ...
    content {
      ...
      dynamic "rule" {
        for_each = [for r in var.nat_rules : r if r.collection_name == nat_rule_collection.value.name]
        content {
          name                = rule.value.name
          source_addresses    = rule.value.source_addresses
          source_ip_groups    = rule.value.source_ip_groups
