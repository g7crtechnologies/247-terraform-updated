...
data "azurerm_lb" "lb" {
  name                = var.lb_name
  resource_group_name = "tfs-rg-${terraform.workspace}-${local.location}-${var.product}"
}

resource "azurerm_lb_backend_address_pool" "lb_backend" {
  for_each        = var.LbRules
  name            = "tfs-ilb-${terraform.workspace}-${each.value["pci"]}-${local.location}-bckpool${each.value["Id"]}"
  loadbalancer_id = data.azurerm_lb.lb.id
}

resource "azurerm_lb_probe" "lb_probe" {
  for_each        = var.LbRules
  name            = "tfs-ilb-${terraform.workspace}-${each.value["pci"]}-${local.location}-probe-${each.value["Id"]}"
...
resource "azurerm_lb_rule" "lb_rule" {
  for_each                       = var.LbRules
  name                           = "tfs-ilb-${terraform.workspace}-${local.location}-rule-${each.value["Id"]}"
  protocol                       = "Tcp"
